...
sed -i 's/\$DOMAIN/'$DOMAIN'/g' /etc/bind/db.$DOMAIN /etc/bind/named.conf.local
...
INSTALL_PACKAGES="zimbra-core zimbra-store zimbra-apache"
EOF

# --------------------------------------------------------
# wget -O /opt/zimbra-install/zimbra-zcs-8.7.7.tar.gz https://files.zimbra.com/downloads/8.7.7_GA/zcs-8.7.7_GA_1787.UBUNTU16_64.20170410133400.tgz
# tar xzvf /opt/zimbra-install/zimbra-zcs-8.7.7.tar.gz -C /opt/zimbra-install/

echo "Extracting files from the archive"
tar xzvf /opt/zimbra-install/zimbra-zcs-*.tar.gz -C /opt/zimbra-install/
